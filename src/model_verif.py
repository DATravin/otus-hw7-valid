...
def get_model(model_name: str, model_stage="Production"):
...
#     train_sdf = datamart(train_dates,row_sdf,agg_cust_sdf,agg_term_sdf,list_for_fillna,sample_val = 0.5)
    ...
    bucket_name = 'cold-s3-bucket'
    output_table_path = f"s3a://{bucket_name}/output_data/temp_verif.parquet"
    ...
    prod_model = get_model(model_name)
    
    stage_model = get_model(model_name,'Staging')
...
#     model_versions
    ...
    if p_value < alpha:
        ...
        if np.mean(vec2)>np.mean(vec1):
            ...
            mlflow_change_stage(model_name, ver_stage, 'Production')
            mlflow_change_stage(model_name, ver_prod, 'Archived')
        
        elif np.mean(vec2)<np.mean(vec1): 
            logger.info(f"old model is better")
            mlflow_change_stage(model_name, ver_stage, 'Archived')
